    ...
    <nav className="w-screen bg-black fixed h-26 z-50 px-3 justify-around drop-shadow-xl text-white flex items-center">
      ...
      >
        <span className="transition duration-300 ease-in-out text-black group-hover:text-white">
          {token ? (location.pathname === "/events" ? "Create Event" : "Logout") : "Login"}
        </span>
        {token ? (
          location.pathname === "/events" ? (
            ...
            <LuLogOut className="text-white opacity-0 group-hover:opacity-100 transform group-hover:translate-x-0 transition duration-300 ease-in-out translate-x-5" />
          )
        ) : (
