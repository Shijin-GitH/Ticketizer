    ...
    if (savedFields) {
      ...
      fields.forEach((field) => {
        if (field.type === "checkbox" && field.options) {
          initialValues[field.id] = [] // Initialize as empty array for multi-checkboxes
        } else if (field.type === "checkbox") {
          initialValues[field.id] = false // Default for single checkbox
        } else if (field.type === "radio" || field.type === "select") {
          initialValues[field.id] = "" // Default for radio and select
        } else {
          initialValues[field.id] = "" // Default for other fields
        }
      })
    ...
    if (step === 1) {
      ...
      const isValid = requiredFields.every((field) => {
        if (field.type === "checkbox" && field.options) {
          ...
          return formValues[field.id] !== undefined && formValues[field.id] !== ""
        }
      })
      ...
      const allMembersValid = teamMembers.every(
        (member) => member.name?.trim() && member.email?.trim() && member.phone?.trim(),
      )
      if (!allMembersValid) {
    ...
    <div className="min-h-screen z-50 text-white">
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="flex items-center justify-between max-w-2xl mx-auto mb-8">
          {["Personal Info", "Team Details", "Ticket Selection", "Payment"].map((label, index) => (
            <div key={index} className="flex items-center">
              <div className="flex flex-col items-center">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center ${
                    step >= index + 1 ? "bg-[#90FF00] text-black" : "bg-gray-700 text-white"
                  }`}
                >
                  {index + 1}
                </div>
              ...
              {index < 3 && (
                <div className="flex-1 h-1 bg-gray-700 mx-2">
                  <div
                    className={`h-full ${step >= index + 2 ? "bg-[#90FF00]" : "bg-gray-700"}`}
        ...
        <div className="max-w-2xl mx-auto bg-gray-900 z-50 rounded-lg border border-gray-800 p-6">
              ...
              <div className="space-y-4">
                {formFields.map((field) => (
                  <div key={field.id} className="mb-4">
                        ...
                        {field.options?.map((option, index) => (
                          <label key={index} className="flex items-center space-x-2 text-white">
                            <input
                              ...
                              checked={formValues[field.id] === (typeof option === "object" ? option.value : option)}
                              onChange={(e) => handleInputChange(field.id, e.target.value)}
                              required={field.required}
                            ...
                            <label key={index} className="flex items-center space-x-2 text-white">
                              <input
                                ...
                                checked={
                                  ...
                                  formValues[field.id].includes(typeof option === "object" ? option.value : option)
                                }
                                onChange={(e) => {
                                  ...
                                  const currentValues = Array.isArray(formValues[field.id])
                                    ...
                                    : []
                                  if (e.target.checked) {
                                    handleInputChange(field.id, [...currentValues, value])
                                  } else {
                                    handleInputChange(
                                      field.id,
                    ...
                    {field.instructions && <p className="mt-1 text-xs text-gray-400">{field.instructions}</p>}
                  </div>
                ))}
              </div>
            </>
              ...
              <div className="space-y-4">
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    onChange={(e) => handleInputChange("team-leader-name", e.target.value)}
                    required
                  />
                </div>
                <div className="mb-4">
                  ...
                  <input
                    ...
                    onChange={(e) => handleInputChange("team-phone", e.target.value)}
                    required
                  />
                </div>
                <div className="mb-4">
                  ...
                  <input
                    ...
                    onChange={(e) => handleInputChange("team-email", e.target.value)}
                    required
                  />
                </div>
                ...
                {tickets.map((ticket) => (
                  ...
                  >
                    <div className="flex justify-between items-center">
                      ...
                      <div className="text-xl font-semibold">₹ {ticket.price}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="flex justify-between mt-8">
            ...
            <button
              ...
              onClick={handleNext}
            >
              {step === 4 ? "Proceed to Payment" : "Next"} <FaArrowRight className="ml-2" />
            </button>
          </div>
        </div>
      </div>
